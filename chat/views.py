...
def api_mensajes_no_leidos(request):
    ...
    if total_no_leidos is None:
        # Optimización: usar una sola consulta con agregación
        from django.db.models import Count, Q
        
        # Buscar conversaciones donde el usuario participa y contar mensajes no leídos
        total_no_leidos = Mensaje.objects.filter(
            Q(conversacion__participante1=user) | Q(conversacion__participante2=user),
            leido=False
        ).exclude(
            remitente=user  # Excluir mensajes enviados por el usuario
        ).count()
        
        # Guardar en caché por 30 segundos
        cache.set(cache_key, total_no_leidos, 30)
